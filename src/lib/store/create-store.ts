...
import type { NodeTypes, EdgeTypes, Node, Edge, FitViewOptions, InternalNode } from '../types';
import { addEdge as addEdgeUtil } from '../utils/edges';
import { initialNodeTypes, initialEdgeTypes } from './initial-types';
import { NgFlowStore } from './flow-store';
import type { FlowStore, NgFlowStoreActions, StoreSignals } from './types';

/**
 * Builds the reactive store and attaches the imperative actions. Ported from
 * Svelte Flow's `createStore`.
 */
export function createStore<NodeType extends Node = Node, EdgeType extends Edge = Edge>(
  ...
  const store = new NgFlowStore<NodeType, EdgeType>(signals);
  ...
  function updateNodeInternals(updates: Map<string, InternalNodeUpdate>) {
    ...
    for (const change of changes) {
      ...
      const node = { ...userNode } as NodeType;
  ...
  function fitView(options?: FitViewOptions<NodeType>) {
    ...
    // Trigger adoptUserNodes by replacing the nodes array reference.
  ...
  function handleNodeSelection(id: string, unselect?: boolean, nodeRef?: HTMLElement | null) {
  ...
  function cancelConnection() {
    store._connection = initialConnection as ConnectionState<InternalNode<NodeType>>;
  }
  ...
  const actions: NgFlowStoreActions<NodeType, EdgeType> = {
    setNodeTypes,
    setEdgeTypes,
    addEdge,
    updateNodePositions,
  ...
  return Object.assign(store, actions) as FlowStore<NodeType, EdgeType>;
}
